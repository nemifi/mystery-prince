...
const catalog = [
  { file: './content/episode-a.json', className: 'hotel', eyebrow: 'EPISODE 01', title: 'THE 23:30 MESSAGE', hook: '23:30に届いた死者からのメッセージ。信じたい男ほど、嘘をついている。', roles: ['REI — ホテル警備責任者','MINATO — 主治医','KAI — イリュージョニスト'] },
  { file: './content/episode-b.json', className: 'train', eyebrow: 'EPISODE 02', title: 'THE SEALED EXPRESS', hook: '封印された宝石は、いつ偽物になったのか。同じ3人が、別の人生を演じる。', roles: ['REI — 美術展学芸員','MINATO — 鉄道捜査官','KAI — 主任車掌'] }
];
...
const log = (type, payload = {}) => {
  const row = { at: new Date().toISOString(), sessionId, episode: episode?.id || null, type, ...payload };
  const rows = JSON.parse(localStorage.getItem('mp_concept_log') || '[]');
  rows.push(row);
  localStorage.setItem('mp_concept_log', JSON.stringify(rows.slice(-500)));
...
function renderCatalog(){
  episode = null; eventIndex = 0; evidence = []; feedback = '';
  app.innerHTML = `<section class="episode-select">
    <header class="hero-head"><p class="eyebrow">CONCEPT TEST / H1 + H2</p><h1>MYSTERY PRINCE</h1><p>同じ彼に、何度でも、はじめて会う。</p></header>
    <div class="episode-grid">${catalog.map((item,i)=>`<button class="episode-card ${item.className}" data-index="${i}">
      <span class="eyebrow">${item.eyebrow}</span><h2>${item.title}</h2><p>${item.hook}</p>
      <div class="role-row">${item.roles.map(r=>`<span>${r}</span>`).join('')}</div>
    </button>`).join('')}</div>
    <p class="log-note">Prototype events are stored locally in this browser for test review.</p>
  </section>`;
...
async function loadEpisode(file){
  ...
  eventIndex = 0; evidence = []; feedback = '';
...
function renderAccuse(ev,scene){
  ...
  shell(scene,main,episode.cast.map(c=>`<button class="action ${c.id===ev.correct?'primary':''}" data-accuse="${c.id}">${escapeHtml(c.name)} — ${escapeHtml(c.role)}</button>`).join(''),`${eventIndex+1}/${episode.events.length}`);
...
function renderEnd(ev,scene){
  ...
  shell(scene,main,`<button class="action primary" data-finish>作品選択へ</button>`,`${episode.events.length}/${episode.events.length}`);
